...
def test_download_pretrained_weights_prints_progress(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        "fungtion.setup_models.urlopen",
        lambda _url: FakeResponse(b"fake-weights"),
    )

    download_pretrained_weights(model_dir=tmp_path / "models")

    captured = capsys.readouterr()
    assert "Downloading ESM-1b weights:" in captured.out
    assert "100.0%" in captured.out


def test_resolve_pretrained_weights_path_returns_none_without_model(
    monkeypatch, tmp_path
):
    monkeypatch.setenv("FUNGTION_MODEL_DIR", str(tmp_path / "models"))
    default_path = get_default_pretrained_weights_path()
