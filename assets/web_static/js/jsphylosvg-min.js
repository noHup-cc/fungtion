...
Smits.PhyloCanvas.Render.Phylogram.prototype = {
	...
	function renderBackground(){
		...
		//bgLabelsHold = bold,1,bold,4.779,1,rich,208.845;
		var arr = [];
		...
		if(bgLabelsHold.length > 0){
			...
			for(var i = 0; i < bgLabelsHold.length; i++){
				...
				//alert(bgLabelsHold[i][2]);
//				if(i != bgLabelsHold.length - 1 && bgLabelsHold[i][0] == bgLabelsHold[(i+1)][0]){
//					alert(bgLabelsHold[i][0]);
//					//alert(bgLabelsHold[(i+1)][2]);
...
//				}
				...
				var arr = sector(
					...
					bgLabelsHold[i][1] - scaleAngle/2,    //rotation degree begin
					bgLabelsHold[i][1] + scaleAngle/2     //rotation degree end
				);	
				//alert(attr.type);
