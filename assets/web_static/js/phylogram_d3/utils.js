...
function formatTree(nodes, links, yscale=null, xscale=null, height, opts) {
    ...
    */
    ...
    d3.selectAll('.leaf circle')
            .attr('cursor','pointer')  //add hand
            .on('mouseover',tip.show)
            /*.on('mouseover', eval(function(d){
            	console.log(options.mapping.get("Highlight").get(d.name))
            	return "tip.show"})) */
            ...
            .on('click',function(d){
            		...
            		if(d.name.split("\|")[0].indexOf("tr")>-1){
				    	
            			return window.location.href="https://www.uniprot.org/uniprot/"+d.name.split("\|")[1];
				    }
            		if(d.name.split("\|")[0].indexOf("gi")>-1){
				    	
            			return window.location.href="https://www.ncbi.nlm.nih.gov/protein/"+d.name.split("\|")[1];
				    }
            ...
            //.on('click',function(d){return console.log(d.name)})  //add links
            //parsed[0]._.url._['1297']
            //var parsed = parseMapping(opts.mapping_dat);
            //console.log(parsed[0]._.url._['1297'])
    ...
    node.append("text")
        ...
        .attr("dy", function(d) { return d.children ? -6 : 0.8 })
        .text(function(d) { 
            if (d.children) {
...
function parseMapping(data) {
    ...
    mapParse.forEach(function(k, v) { // v is a d3.set of mapping column values, with leaf ID has key
        ...
        if (typeof vals[0] === 'string' || vals[0] instanceof String) { // ordinal scale
            var tmp = d3.scale.category20().range(["#c23531", "#c8c788", "#61a0a8", "#d48265", "#91c7ae", "#749f83", "#ca8622", "#bda29a", "#6e7074","#c4ccd3","#546570","#759aa0","#e69d87","#dd6b66","#8dc1a9","#ea7e53","#73a373","#73b9bc","#7289ab","#91ca8c","#f49f42","#d87c7c","#919e8b","#d7ab82","#6e7074","#61a0a8","#efa18d"]);
            if (vals.length > 20) {
                tmp = d3.scale.category20().range(["#c23531", "#c8c788", "#61a0a8", "#d48265", "#91c7ae", "#749f83", "#ca8622", "#bda29a", "#6e7074","#c4ccd3","#546570","#759aa0","#e69d87","#dd6b66","#8dc1a9","#ea7e53","#73a373","#73b9bc","#7289ab","#91ca8c","#f49f42","#d87c7c","#919e8b","#d7ab82","#6e7074","#61a0a8","#efa18d"]);
            }
...
*/
function fitTree(page) {

    var y1 = window.innerHeight;
    //console.log(y1)
	var y1 = 1000;
    var x1 = jQuery('#tree').width();
    
    //d3.select('#SVGtree').call(zoom.on("zoom", panZoom));
    
    d3.select('svg').attr("viewBox", "0 0 " + parseInt(x1) + " " + parseInt(y1)).attr("height","1000px");
    //d3.select('svg').attr("style", "height:1000px;weight:100%"); // fit viewbox
    // reset position
//    d3.select('#canvasSVG')
//        .attr('transform','translate(0,0) scale(1)')
        //.attr('padding','1000px 0px 0px 0px')
...
//        (jQuery('#canvas').width() - margin.left - margin.right) / content.width, 
//        (jQuery('#canvas').height() - margin.bottom - margin.top) / content.height
//
//    ]);
    //alert(x1)
    var zoomScale = 1.8;
...
//    console.log(jQuery('#canvas').width() - margin.left - margin.right)
//    console.log(jQuery('#canvas').height() - margin.bottom - margin.top)
//    console.log(content.width)
//    svg.call(zoom.event);
//    
//    zoom.scale(zoomScale-0.2);
//    if (treeType == 'rectangular') {
//        zoom.translate([margin.left,margin.top]);
//    } else {
//    	zoom.translate([x1 / 2, y1 / 2]);
//    	//zoom.translate([jQuery('#canvas').width() / 2, jQuery('#canvas').height() / 2]);
//        //zoom.translate([x1 / 2, (root.bottom - content.top) * zoom.scale()]);
//        //zoom.translate([x1 / 2, (root.bottom + 25 - content.top) * zoom.scale()]); //400 control the distance from the top
//    }
    //svg.call(zoom.on("zoom", null));
    //console.log('translate('+(x1 / 2)+','(y1 / 2)+') scale('+(zoomScale-0.2)+')')
    d3.select('#canvasSVG')
    .attr('transform','translate('+(x1 / 2)+','+(y1 / 2)+') scale('+zoomScale+')')
...
function buildGUI(page,selector, opts) {
...
//    .style("margin-top","10px");
    ...
    tmp.append("button")
        ...
        .attr("onclick","options.treeType = this.id; updateTree();fitTree();")
    ...
    if (mapParse && !mapParse.empty() && typeof mapParse != 'undefined') {
        ...
        var col2 = guiRow1.append("div")
            ...
            //.attr("style","display:none")             //hide div
...
//            	console.log(d);
        ...
        if(page==1){
            ...
            .attr("selected","SpeciesFirst")
            .attr("value","SpeciesFirst")
            //.text('None');
        	.text('SpeciesFirst');
        }else{
        	select1.append("option")
...
function formatTooltip(d, mapParse) {
	...
	var html = "<div class='tip-title' style='margin-bottom: 10px;'><span class='tip-meta-name'>" + d.name + "</span></div>";
    if (mapParse) {
...
//            .attr("opacity",0) //add by dw
    ...
    var legendRow = legend.selectAll('g.legend')
        ...
        .append('g')
            .attr('class', 'legend')
            .attr('transform', function(d,i) { return 'translate(25,' + (25 + i * 30) + ')'; } )
...
function positionLegend() {
    ...
    d3.select("#legendID").attr("transform","translate(" + 0 + "," + 5 + ")");
}
...
function updateLegend() {
    ...
    if (options.leafColor != '') {
        ...
        svg.selectAll('g.leaf.node circle')
            ...
            .style('fill', function(d) {
            	...
            	return mapVals.get(d.name) ? colorScale(mapVals.get(d.name)) : 'black'
            })
    ...
    if (options.backgroundColor != '') {
        var colorScale = options.colorScale.get(options.backgroundColor) // color scale
        var mapVals = options.mapping.get(options.backgroundColor) // d3.map() obj with leaf name as key
...
//            })
//            .style('opacity',1)
        svg.selectAll('g.leaf.node text.leafLabel')
        	.attr("style",function(d) {
//        		console.log(d)
        		return mapVals.get(d.name)!="-" ? "fill:red;font-weight:bold" : 'none'
        		//return d.name!="-" ? "fill:red;font-weight:bold" : 'none'
              })
